...
bool ReadFileToString(StringPiece name, string* output) {
  ...
  Status status;
  std::unique_ptr<file::ReadonlyFile> fl(CHECK_NOTNULL(res.obj));
  strings::Slice slice;
  size_t offset = 0;
  size_t sz = fl->Size();
  while (offset < sz) {
...
base::Status StatFilesSafe(StringPiece path, std::vector<file::StatShort>* res) {
  ...
  if (rv && rv != GLOB_NOMATCH) {
    return base::Status(base::StatusCode::IO_ERROR, strerror(rv));
  }
...
std::vector<file::StatShort> StatFiles(StringPiece path) {
  ...
  base::Status rv = StatFilesSafe(path, &res);
  CHECK(rv.ok()) << rv;
  return res;
}

void CompressToGzip(StringPiece file, uint8_t compress_level) {
  base::StatusObject<file::ReadonlyFile*> src = file::ReadonlyFile::Open(file);
  CHECK(src.ok()) << file;
  ...
  StringPiece result;
  while (true) {
    CHECK_STATUS(src.obj->Read(offs, kBufSize, &result, buf.get()));
    if (result.empty())
      break;

    CHECK_GT(gzwrite(dest, result.data(), result.size()), 0);
    if (result.size() < kBufSize)
      break;
    offs += result.size();
  }
...
// false - file, true - dir.
bool IsFileDirOrDie(const string& path) {
  struct stat s;
  CHECK_EQ(0, stat(path.c_str(), &s));
  if (s.st_mode & S_IFDIR) {
    return true;
  } else if (s.st_mode & S_IFREG) {
    return false;
  } else {
    LOG(FATAL) << path << " is not file neither dir";
  }
}
