...
// Returns whether v has a null byte inside.
inline constexpr bool haszero(uint32 v) {
  return (v - 0x01010101UL) & ~v & 0x80808080UL;
}

inline constexpr bool hasvalue(uint32 v, uint8 n) {
  return haszero(v ^ (~0UL/255 * n));
}

static JsonParser::Status parse_string(StringPiece str, JsonObject::Value* val) {
  /* Skip starting quote */
  size_t parts = (str.size() - 1) / 4;
  const char* start = str.data() + 1;
  const char* next = start;
  for (unsigned k = 0; k < parts; ++k, next += 4) {
    uint32 val = UNALIGNED_LOAD32(next);
    if (hasvalue(val, '"') || hasvalue(val, '\\'))
      break;
  }
  ...
  for (; next != end; ++next) {
    ...
    if (c == '\"') {
      ...
      val->u.token.set(start, next - start);
      return JsonParser::SUCCESS;
    }
    ...
    if (c == '\\') {
      ++next;
      // For some reason if I remove the switch the code becomes slower :(
      switch (*next) {
