...
void SingleProducerTaskPoolBase::PrintSlowTask(uint32 ms_delay, MicrosecondsInt64 start) {
  LOG(WARNING) << "task from " << start << " was slowly awaken after " << ms_delay << " msec";
}
  ...
  while (!me->start_cancel_) {
    ...
    if (++num_yields > 100) {
      ...
      if (thread_interface->IsQueueEmpty()) {
        MicrosecondsInt64 start = base::GetMonotonicMicrosFast();
