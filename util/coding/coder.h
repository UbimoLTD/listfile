...
class GrowableEncoder {
  std::string buf_;
public:
  GrowableEncoder& put_varint32(uint32 v) {
    Varint::Append32(&buf_, v);
    return *this;
  }

  GrowableEncoder& put_string(const std::string& s) {
    buf_.append(s);
    return *this;
  }

  const uint8* byteptr() const { return reinterpret_cast<const uint8*>(buf_.data()); }
...
inline void Encoder::put_varint32(uint32 v) {
  buf_ = Varint::Encode32(buf_, v);
}

inline void Encoder::put_varint64(uint64 v) {
  buf_ = Varint::Encode64(buf_, v);
}
