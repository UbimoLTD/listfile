...
extern const char kBuildTagString[];
extern const char kBuildTaggerString[];
...
#define WARN_IF_ERROR(stmt) \
  do { \
    ::base::Status __status__ = (stmt); \
    if (CPU_PREDICT_FALSE(!__status__.ok())) \
      LOG(WARNING) << "Failure running " << #stmt << " got error " << __status__; \
  } while (false)
