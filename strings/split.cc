...
// FixedLength
//
FixedLength::FixedLength(uint32 length) : length_(length) {
  CHECK_GT(length, 0U);
}
...
static const uint32 kMaxOverCut = 12;
// The ellipsis to add to strings that are too long
static const char kCutStr[] = "...";
static const uint32 kCutStrSize = sizeof(kCutStr) - 1;

// ----------------------------------------------------------------------
// Return the place to clip the string at, or -1
// if the string doesn't need to be clipped.
// ----------------------------------------------------------------------
static int ClipStringHelper(const char* str, uint32 max_len, bool use_ellipsis) {
  if (strlen(str) <= max_len)
    return -1;

  uint32 max_substr_len = max_len;
...
void ClipString(char* str, uint32 max_len) {
...
// ----------------------------------------------------------------------
void ClipString(string* full_str, uint32 max_len) {
...
void SplitStringToLines(const char* full,
  ...
  uint32 pos = 0;
  for (uint32 i = 0; (i < num_lines || num_lines <= 0); i++) {
