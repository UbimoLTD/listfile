...
void JoinStringsInArray(const std::string* components,
                        int num_components,
                        const char* delim,
                        std::string* result);
void JoinStringsInArray(const std::string* components,
...
// ----------------------------------------------------------------------
template<typename C> string JoinNumbers(const C& c, const StringPiece delim) {
  string result;
  for (const auto& item : c) {
    StrAppend(&result, AlphaNum(item).data(), delim.data());
  }
  if (!result.empty())
...
void JoinStringsIterator(const ITERATOR start,
  ...
  if (start != end) {
    ...
    result->reserve(length + (count - 1) * delim.size());
  }
