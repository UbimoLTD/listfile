...
//
//
// Modified by roman@ubimo.com (Roman Gershman)
//
...
class FixedLength {
 public:
  explicit FixedLength(uint32 length);
  StringPiece Find(StringPiece text) const;
...
struct AllowEmpty {
  bool operator()(StringPiece) const {
    return true;
  }
};
...
bool SplitStringAndParse(StringPiece source, StringPiece delim,
                         bool (*parse)(StringPiece str, T* value),
                         vector<T>* result);
template <class Container>
bool SplitStringAndParseToContainer(
    StringPiece source, StringPiece delim,
    bool (*parse)(StringPiece str, typename Container::value_type* value),
    Container* result);

template <class List>
bool SplitStringAndParseToList(
    StringPiece source, StringPiece delim,
    bool (*parse)(StringPiece str, typename List::value_type* value),
    List* result);
// ----------------------------------------------------------------------
// SplitRange()
