...
int EditDistance(StringPiece s1, StringPiece s2) {
  if (s1.length() == 0)
    return s2.length();
  ...
  // Init matrix.
  std::vector<vector<int>> m(s1.length() + 1, std::vector<int>((s2.length() + 1),0));
  ...
  // Fill matrix.
  for (unsigned int i1 = 1; i1 <= s1.length(); i1++) {
    for (unsigned int i2 = 1; i2 <= s2.length(); i2++) {
      ...
      // Char change option.
      int chn = m[i1-1][i2-1] + (s1[i1 - 1]==s2[i2 - 1]? 0 : 1);

      m[i1][i2] = ins < del? std::min(ins, chn) : std::min(del, chn);
    }
  }
